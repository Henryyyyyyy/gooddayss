package com.gooddays.zj.module_base.widge.statusbar;
...
public class StatusBarUtil {
    ...
    public static void setWhiteStatusBar(Activity activity) {
        setColor(activity, 0xffffffff, DEFAULT_STATUS_BAR_ALPHA);
    }
    /**
     * 设置状态栏颜色
     *
     ...
     * 安卓6.0状态栏深色字体
     */
    public static boolean androidStatusBarLightMode(Activity activity, boolean isLiveHome) {
        boolean MIUI = StatusBarUtil.MIUISetStatusBarLightMode(activity.getWindow(), true);
        boolean flyMe = false;
        if (!MIUI) {
            flyMe = StatusBarUtil.FlymeSetStatusBarLightMode(activity.getWindow(), true);
        }
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (!MIUI && !flyMe && isLiveHome){
                return true;
            }
        } else {
            if (!flyMe && !MIUI) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) { //4.4以上可以设置透明
                    StatusBarUtil.setTransparentForImageView(activity, null);
                    return isLiveHome;
                } else {
                    setColor(activity, Color.parseColor("#EBECF0"), 0);
                }
            } else {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) { //4.4以上可以设置透明
                    if (isLiveHome) {
    ...
    /**
     * 主要原理是从系统R文件中找到dimen这个内部类，然后通过反射拿到dimen中的status_bar_height的值
     */
